// import the Metrics class from the model folder
...
export class CopilotUsageChecker {
    ...
    // add new function to check the total_lines_suggested, it should be greater than total_accepted_suggestions,and less than total_lines
    private checkTotalLinesSuggested(): void {
        this.data.forEach(item => {
            if (item.total_lines_suggested < item.total_acceptances_count || item.total_lines_suggested > item.total_lines_accepted) {
                this.zeroActivityDays.push(item.day);
            }
        });
    }

    // check the break down of the total_lines_suggested, it should be equal to the sum of the total_lines_suggested of each breakdown
